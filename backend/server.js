...
const dayCards = [
  { title: 'Молот Творца', text: 'Сегодня вы обладаете силой создавать и изменять. Используйте эту энергию мудро!' },
  { title: 'Кристалл Мудрости', text: 'Ясность мысли поможет найти решение любой задачи. Доверьтесь интуиции.' },
  { title: 'Щит Защиты', text: 'Вы под надежной защитой. Смело идите вперед, не боясь препятствий.' },
  { title: 'Зелье Удачи', text: 'Фортуна благосклонна к вам! Хороший день для новых начинаний.' },
  { title: 'Ключ Возможностей', text: 'Перед вами открываются новые двери. Будьте готовы к переменам.' }
];

const adviceTexts = [
  'Помните: каждый гном начинал с малого. Великие дела складываются из маленьких шагов.',
  'Мудрый гном знает: лучше один раз увидеть, чем сто раз услышать. Действуйте!',
  'В кузнице жизни каждый удар молота важен. Не пренебрегайте мелочами.',
  'Золото не ржавеет, а доброта не стареет. Будьте добрыми к окружающим.'
];
...
app.post('/api/day-card', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Не удалось получить карту дня' });
  }
});
...
app.get('/api/advice', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Не удалось получить совет' });
  }
});
...
app.post('/api/numerology', (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Не удалось рассчитать нумерологию' });
  }
});
...
app.post('/api/compatibility', (req, res) => {
  try {
    ...
    if (!sign1 || !sign2) {
      return res.status(400).json({ error: 'Оба знака зодиака обязательны' });
    }
    
    const compatibility = Math.floor(Math.random() * 40) + 60; // 60-100%
    let emoji = compatibility >= 90 ? '💖' : compatibility >= 80 ? '💕' : compatibility >= 70 ? '❤️' : '💛';
    
    let description;
    if (compatibility >= 90) description = 'Идеальная пара! Звезды благословляют ваш союз.';
    else if (compatibility >= 80) description = 'Отличная совместимость! Вы прекрасно дополняете друг друга.';
    else if (compatibility >= 70) description = 'Хорошие отношения! Немного усилий - и будете неразлучны.';
    ...
    res.status(500).json({ error: 'Не удалось рассчитать совместимость' });
  }
});
...
app.get('/api/mercury', (req, res) => {
  try {
    ...
    res.json({
      isRetrograde,
      period: isRetrograde ? 'Ретроградный до конца месяца' : 'Директное движение',
      description: isRetrograde 
        ? 'Меркурий ретроградный. Время переосмысления и завершения дел. Будьте внимательны с техникой.'
        : 'Меркурий в директном движении. Отличное время для новых начинаний и переговоров.',
      date: new Date().toISOString(),
      source: 'internet'
    });
    ...
    res.status(500).json({ error: 'Не удалось получить статус Меркурия' });
  }
});
